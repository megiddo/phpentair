...
RUN apt-get update && apt-get install -y less gzip wget nginx zip unzip git nano mysql-client cron sqlite3
...
RUN mkdir -p /var/www/pentair/app
RUN chown -R www-data:www-data /var/www/pentair

COPY nginx.app.config /etc/nginx/sites-available/pentair
